...
def ratesEndcap_list():
  ...
  for r in ring:
    for roll in rolls:
      ...
      for c in chambers:
        try:
          if float(rates1["rate"]["RE+2"+r+"_CH"+c+roll]["ratesquarecm"]) == 0: continue
          rates.append(float(rates1["rate"]["RE+2"+r+"_CH"+c+roll]["ratesquarecm"]))
          names.append("RE+2"+r+"_CH"+c+roll)
        except KeyError:
          continue
...
def ratesWheel_list():
  ...
  parameters = ["W+"+w+"_RB2"+s+"_S"+c+"_"+ri for w in wheels
...
def average_chambers(wheel, dictionary, X, Y):
  if wheel == 1:
      ...
      for s in subrolls:
        for ri in ring:
          eta = [v["eta"] for k, v in dictionary.items() 
                 for c in chambers if k == "W+"+w+"_RB2"+s+"_S"+c+"_"+ri]
    ...
    for r in ring:
      for roll in rolls:
        eta = [v["eta"] for k, v in dictionary.items() for c in chambers if k == "RE+2"+r+"_CH"+c+roll]
        rates = [v["rates"] for k, v in dictionary.items() for c in chambers if k == "RE+2"+r+"_CH"+c+roll]
        if rates and len(rates) != 1:
          X.append( sum(eta)/len(eta) )
          Y.append( sum(rates)/len(rates) )
...
def main():
  ...
  gr.SetTitle( 'Pseudorapidity Distribution for Section 2' )
  ...
  gr.Draw("AP")
  canv.SaveAs("etaDistroSection2.gif")

...
def main():
  ...
  doext = bool(int(options.doext))
  newWp = bool(int(options.newwp))
  minrun = 190000
  maxrun = 400000
  maxlum = 20000.0
  maxtime = 7200.0
  ...
  torootFile(alldics,options.outfile)
  ...
  rateEvalWp2 = printmultigraph(wp2gr, rbsort,  plotoptionsRBs, options.outfile+"_RB_Wp2", "W+2", year, doext, False, rbmax)
...
def printmultigraph( grs, sortk, plotoptions, outn, title, year, doext, ploterror, ymax):
  ...
  leg = TLegend(0.345,0.58,0.645,0.88) #TLegend(1. - c.GetRightMargin() - 0.8, 1. - c.GetTopMargin() - 0.40,1. - c.GetRightMargin()- 0.60, 1. - c.GetTopMargin() -0.02)
  leg.SetFillStyle( 0 )
  leg.SetBorderSize(0)
  leg.SetMargin( 0.1 )
  #ymax = 0;

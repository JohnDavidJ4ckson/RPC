...
def main():
  ...
  for s in sections:
    ...
    queryAfter  = "cbrpc_lumi_feed -d DPID2 -wd 'RE-4' -c '"+s1+"' '"+s2+"' '"+s3+"' --lhcfill 7259 > queues/afterTS2ch"+s+".csv"
    executeAutomat = "cbrpc_lumi start"
    restart = "cd /afs/cern.ch/work/d/dperezna/public/CMSSW_9_2_15/src/RPC/currentPlots/RSforRE+4Crosscheck"
